...
impl PlatformBackend {
  ...
  pub async fn activate(&mut self) -> Result<(), XosmsError> {
    self.platform_data.connection = Some(
  ...
  pub async fn update(&self, state: MediaPlayerState) -> Result<(), XosmsError> {
    let mut changes = Vec::new();
    {
      ...
      if stale_state.play_button_enabled != state.play_button_enabled {
        changes.push(MediaPlayerStatePropertyChange::PlayButtonEnabled);
      }
      if stale_state.pause_button_enabled != state.pause_button_enabled {
        changes.push(MediaPlayerStatePropertyChange::PauseButtonEnabled);
      }
      if stale_state.stop_button_enabled != state.stop_button_enabled {
        changes.push(MediaPlayerStatePropertyChange::StopButtonEnabled);
      }
      if stale_state.previous_button_enabled != state.previous_button_enabled {
        changes.push(MediaPlayerStatePropertyChange::PreviousButtonEnabled);
      }
      if stale_state.next_button_enabled != state.next_button_enabled {
        changes.push(MediaPlayerStatePropertyChange::NextButtonEnabled);
      }
      if stale_state.seek_enabled != state.seek_enabled {
        changes.push(MediaPlayerStatePropertyChange::SeekEnabled);
      }
      if stale_state.fullscreen != state.fullscreen {
        changes.push(MediaPlayerStatePropertyChange::Fullscreen);
      }
      if stale_state.playback_rate != state.playback_rate {
        changes.push(MediaPlayerStatePropertyChange::PlaybackRate);
      }
      if stale_state.minimum_playback_rate != state.minimum_playback_rate {
        changes.push(MediaPlayerStatePropertyChange::MinimumPlaybackRate);
      }
      if stale_state.maximum_playback_rate != state.maximum_playback_rate {
        changes.push(MediaPlayerStatePropertyChange::MaximumPlaybackRate);
      }
      if stale_state.shuffle != state.shuffle {
        changes.push(MediaPlayerStatePropertyChange::Shuffle);
      }
      if stale_state.r#loop != state.r#loop {
        changes.push(MediaPlayerStatePropertyChange::Loop);
      }
      if stale_state.volume != state.volume {
        changes.push(MediaPlayerStatePropertyChange::Volume);
      }
      if stale_state.playback_status != state.playback_status {
        changes.push(MediaPlayerStatePropertyChange::PlaybackStatus);
      }
      let mut metadata_changed = false;
      if stale_state.title != state.title {
        metadata_changed = true;
      }
      if stale_state.artist != state.artist {
        metadata_changed = true;
      }
      if stale_state.album_title != state.album_title {
        metadata_changed = true;
      }
      if stale_state.track_id != state.track_id {
        metadata_changed = true;
      }
      if (stale_state.thumbnail.is_none() && state.thumbnail.is_some())
        || (stale_state.thumbnail.is_some() && state.thumbnail.is_none())
      {
        metadata_changed = true;
      }
      if stale_state.thumbnail.is_some()
        && state.thumbnail.is_some()
        && let Some(stale_thumbnail) = stale_state.thumbnail.as_ref()
        && let Some(thumbnail) = state.thumbnail.as_ref()
      {
        if stale_thumbnail.id != thumbnail.id {
          metadata_changed = true;
        }
      }
      if metadata_changed {
        changes.push(MediaPlayerStatePropertyChange::Metadata);
      }
    ...
    if let Some(connection) = &self.platform_data.connection {
      let mediaplayer2_iface = connection
        .object_server()
        .interface::<_, MPRISMediaPlayer2>("/org/mpris/MediaPlayer2")
        .await?;
      let mediaplayer2_player_iface = connection
        .object_server()
        .interface::<_, MPRISMediaPlayer2Player>("/org/mpris/MediaPlayer2")
        .await?;
      ...
      for change in changes {
        match change {
          MediaPlayerStatePropertyChange::PlayButtonEnabled => {
            mediaplayer2_player
              .can_play_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::PauseButtonEnabled => {
            mediaplayer2_player
              .can_pause_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::StopButtonEnabled => {} // Tied to CanControl which is always true. Ignored on this platform
          MediaPlayerStatePropertyChange::PreviousButtonEnabled => {
            mediaplayer2_player
              .can_go_previous_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::NextButtonEnabled => {
            mediaplayer2_player
              .can_go_next_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::SeekEnabled => {
            mediaplayer2_player
              .can_seek_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::Fullscreen => {
            mediaplayer2
              .fullscreen_changed(mediaplayer2_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::PlaybackRate => {
            mediaplayer2_player
              .rate_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::MinimumPlaybackRate => {
            mediaplayer2_player
              .minimum_rate_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::MaximumPlaybackRate => {
            mediaplayer2_player
              .maximum_rate_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::Shuffle => {
            mediaplayer2_player
              .shuffle_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::Loop => {
            mediaplayer2_player
              .loop_status_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::Volume => {
            mediaplayer2_player
              .volume_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::PlaybackStatus => {
            mediaplayer2_player
              .playback_status_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
          MediaPlayerStatePropertyChange::Metadata => {
            mediaplayer2_player
              .metadata_changed(mediaplayer2_player_iface_emitter)
              .await?
          }
        }
      }
  ...
  pub async fn set_timeline(&mut self, duration: f64, position: f64) -> Result<(), XosmsError> {
    ...
    let stale_duration = self
      .timeline_state
      .duration
      .swap(duration, Ordering::Relaxed);
    let stale_position = self
      .timeline_state
      .position
      .swap(position, Ordering::Relaxed);

    let now = Instant::now();
    let elapsed = now
      .duration_since(self.last_timeline_state_update)
      .as_micros() as f64;
...
impl MPRISMediaPlayer2Player {
  async fn next(&self) {
    ...
    if state.next_button_enabled {
      let _ = self
        .callback_tx
        .try_send(PlatformBackendEvent::ButtonPressed {
  ...
  async fn play_pause(&self) {
    ...
    if state.pause_button_enabled {
      let _ = self
        .callback_tx
        .try_send(PlatformBackendEvent::ButtonPressed {
          button: ButtonPressedType::PlayPause,
        });
    }
  }
  ...
  async fn seek(&self, offset: i64) {
    ...
    if state.seek_enabled {
      let _ = self
        .callback_tx
        .try_send(PlatformBackendEvent::PositionSeeked {
          offset: offset as f64 / 1_000_000.0,
        });
    }
  }
  ...
  async fn set_position(&self, track_id: ObjectPath<'_>, position: i64) {
    ...
    if state.seek_enabled {
      ...
      let _ = self
        .callback_tx
        .try_send(PlatformBackendEvent::PositionChanged {
  ...
  async fn set_loop_status(&self, value: String) {
    ...
    let _ = self
      .callback_tx
      .try_send(PlatformBackendEvent::LoopChanged { loop_type });
  }
  ...
  async fn set_rate(&self, value: f64) {
    let _ = self
      .callback_tx
      .try_send(PlatformBackendEvent::RateChanged { rate: value });
  }
  ...
  async fn set_shuffle(&self, value: bool) {
    let _ = self
      .callback_tx
      .try_send(PlatformBackendEvent::ShuffleChanged { shuffle: value });
  }
  ...
  async fn metadata(&self) -> HashMap<String, Value<'_>> {
    ...
    metadata.insert(
      ...
      Value::from(ObjectPath::try_from(track_id).unwrap()),
    );
    metadata.insert(
      "mpris:length".to_owned(),
      Value::from(self.timeline_state.duration.load(Ordering::Relaxed)),
    );
    if let Some(thumbnail) = state.thumbnail.as_ref() {
      metadata.insert(
        "mpris:artUrl".to_owned(),
        Value::from(thumbnail.platform_data.thumbnail.clone()),
      );
    }
    metadata.insert(
      "xesam:album".to_owned(),
      Value::from(state.album_title.clone()),
    );
  ...
  async fn set_volume(&self, value: f64) {
    let _ = self
      .callback_tx
      .try_send(PlatformBackendEvent::VolumeChanged { volume: value });
  }
