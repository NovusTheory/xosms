...
use crate::{ButtonPressedType, LoopType, MediaPlayerState, MediaPlayerThumbnailType, PlaybackStatus, backends::{PlatformBackend, PlatformThumbnailBackend, common::{MediaPlayerTimelineState, PlatformBackendEvent}, error::{PlatformBackendError, WindowsPlatformError, XosmsError}, windows::bindings::{MediaPlaybackAutoRepeatMode, MediaPlaybackStatus, MediaPlaybackType, MediaPlayer as WindowsMediaPlayer, RandomAccessStreamReference, StorageFile, SystemMediaTransportControls, SystemMediaTransportControlsButton, SystemMediaTransportControlsTimelineProperties, Uri }}};
...
impl PlatformBackend {
    ...
    pub async fn activate(&mut self) -> Result<(), XosmsError> {
        ...
        let callback_tx = self.callback_tx.clone();
        let playback_position_change_requested_revoker = smtc.PlaybackPositionChangeRequested(move |_sender, args| {
            if let Some(args) = &*args {
        ...
        self.platform_data.playback_position_change_requested_revoker = Some(playback_position_change_requested_revoker.into_token());

        let callback_tx = self.callback_tx.clone();
        let shuffle_enabled_change_requested_revoker = smtc.ShuffleEnabledChangeRequested(move |_sender, args| {
            if let Some(args) = &*args {
                if let Ok(requested_shuffle_enabled) = args.RequestedShuffleEnabled() {
                    let _ = callback_tx.try_send(PlatformBackendEvent::ShuffleChanged { shuffle: requested_shuffle_enabled });
                }
            }
        })?;
        self.platform_data.shuffle_enabled_change_requested_revoker = Some(shuffle_enabled_change_requested_revoker.into_token());

        let callback_tx = self.callback_tx.clone();
        let auto_repeat_mode_change_requested_revoker = smtc.AutoRepeatModeChangeRequested(move |_sender, args| {
            if let Some(args) = &*args {
                if let Ok(requested_auto_repeat_mode) = args.RequestedAutoRepeatMode() {
                    let _ = callback_tx.try_send(PlatformBackendEvent::LoopChanged { loop_type: match requested_auto_repeat_mode {
        ...
        self.platform_data.auto_repeat_mode_change_requested_revoker = Some(auto_repeat_mode_change_requested_revoker.into_token());
    ...
    pub async fn set_timeline(&mut self, duration: f64, position: f64) -> Result<(), XosmsError> {
        if let Some(media_player) = self.platform_data.media_player.as_ref() {
            ...
            timeline_props.SetEndTime(TimeSpan::from_micros((duration * 1_000_000.0).round() as i64))?;
            timeline_props.SetPosition(TimeSpan::from_micros((position * 1_000_000.0).round() as i64))?;
            timeline_props.SetMinSeekTime(TimeSpan::ZERO)?;
            timeline_props.SetMaxSeekTime(TimeSpan::from_micros((duration * 1_000_000.0).round() as i64))?;
    ...
    fn unregister_windows_handlers(&mut self, smtc: SystemMediaTransportControls) -> Result<(), XosmsError> {
        ...
        if let Some(token) = self.platform_data.button_pressed_revoker.take() {
            unsafe {
                let result = (windows_core::Interface::vtable(&smtc).RemoveButtonPressed)(smtc.as_raw(), token);
                if result.is_err() {
                    return Err(
                        XosmsError::PlatformBackend(
                            PlatformBackendError::Windows(WindowsPlatformError::HResult(result))
                        )
                    )
                }
            }
        }
        if let Some(token) = self.platform_data.playback_position_change_requested_revoker.take() {
            let this = &windows_core::Interface::cast::<bindings::ISystemMediaTransportControls2>(&smtc)?;
            unsafe {
                let result = (windows_core::Interface::vtable(this).RemovePlaybackPositionChangeRequested)(this.as_raw(), token);
                if result.is_err() {
                    return Err(
                        XosmsError::PlatformBackend(
                            PlatformBackendError::Windows(WindowsPlatformError::HResult(result))
        ...
        if let Some(token) = self.platform_data.shuffle_enabled_change_requested_revoker.take() {
            let this = &windows_core::Interface::cast::<bindings::ISystemMediaTransportControls2>(&smtc)?;
            unsafe {
                let result = (windows_core::Interface::vtable(this).RemoveShuffleEnabledChangeRequested)(this.as_raw(), token);
                if result.is_err() {
                    return Err(
                        XosmsError::PlatformBackend(
                            PlatformBackendError::Windows(WindowsPlatformError::HResult(result))
        ...
        if let Some(token) = self.platform_data.auto_repeat_mode_change_requested_revoker.take() {
            let this = &windows_core::Interface::cast::<bindings::ISystemMediaTransportControls2>(&smtc)?;
            unsafe {
                let result = (windows_core::Interface::vtable(this).RemoveAutoRepeatModeChangeRequested)(this.as_raw(), token);
                if result.is_err() {
                    return Err(
                        XosmsError::PlatformBackend(
                            PlatformBackendError::Windows(WindowsPlatformError::HResult(result))
...
impl Drop for PlatformBackend {
    fn drop(&mut self) {
        if let Some(media_player) = self.platform_data.media_player.take() {
            ...
            eprintln!("[xosms warning] Native memory leak: Failed to free event handlers for PlatformBackend {:p}", self);
        }
    }
}
